...
public class MonsterVision : MonoBehaviour // Отвечает только за зрение монстра
{
    ...
    public bool CanSeePlayer() // Проверить, видит ли монстр игрока
    {
        ...
        if (Physics.Raycast(rayStart, directionToPlayer, out RaycastHit hit, viewDistance, obstacleMask)) // Пускаем луч зрения
        {
